...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++){
    for (unsigned y = 0; y < image.height(); y++){
      ...
      if (distance < 160){
        pixel.l = pixel.l * (1- (distance * 0.5/100));
      }
      else{
        pixel.l = pixel.l * 0.2;
      }
...
PNG illinify(PNG image) {
  ...
  for(unsigned x = 0; x < image.width(); x++){
    for(unsigned y = 0; y < image.height(); y++){
      HSLAPixel & pixel = image.getPixel(x,y);
      int distO = (abs(pixel.h - illiO) < 360 - pixel.h + illiO) ? abs(pixel.h-illiO):(360 - pixel.h + illiO);
      int distB = (abs(pixel.h - illiB) < 360 - pixel.h + illiB) ? abs(pixel.h-illiB):(360 - pixel.h + illiB);
      if (distO < distB){
        pixel.h = illiO;
      }
      else{
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for(unsigned x = 0; x < min(firstImage.width(), secondImage.width()); x++){
    for(unsigned y = 0; y < min(firstImage.height(), secondImage.height()); y++){
        ...
        if(pixel2.l==1){
          pixel.l = (pixel.l + 0.2 < 1.0) ? pixel.l+0.2 : 1.0;
        }
    }
  }
  return firstImage;
